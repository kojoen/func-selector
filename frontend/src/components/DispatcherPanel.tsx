...
import { Send, Play, Binary, Layers, AlertCircle, CheckCircle2 } from "lucide-react";
...
export function DispatcherPanel() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Left: Input Builder */}
      <div className="border border-border rounded-xl bg-card p-6 space-y-5 shadow-card">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-text-secondary hover:text-text bg-bg border border-border rounded-lg px-3 py-1.5 transition hover:border-accent/40"
          >
        ...
        {!rawMode ? (
          <div className="space-y-4">
            ...
            {preset.inputs.length > 0 ? (
              ...
              <p className="text-xs text-text-muted py-3.5 text-center border border-border rounded-lg bg-bg">
                No parameters required. Only 4-byte selector <code className="text-accent font-semibold">{preset.selector}</code> will be dispatched.
              </p>
            )}
          </div>
        ) : (
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 text-xs font-semibold text-white bg-accent hover:bg-accent-hover py-3 rounded-lg transition disabled:opacity-40 shadow-glow-sm"
        >
      ...
      <div className="border border-border rounded-xl bg-card p-6 space-y-5 flex flex-col justify-between shadow-card">
        <div className="space-y-4">
          <div>
          ...
          <div className="space-y-3">
            <div>
              <span className="text-xs text-text-muted font-medium">Target Proxy Address</span>
              <p className="font-mono text-xs text-text-secondary mt-1 break-all bg-bg rounded-lg px-3.5 py-2.5 border border-border">
                {CONTRACT_ADDRESSES.dispatcher}
              </p>
            </div>

            <div>
              <span className="text-xs text-text-muted font-medium">Byte-Level Slicing</span>
              <div className="mt-1 bg-bg rounded-lg p-3.5 border border-border font-mono text-xs space-y-2 break-all">
        ...
        <div className="pt-4 border-t border-border">
          {result ? (
            <div className="bg-ok/10 border border-ok/30 rounded-lg p-3.5 text-xs text-ok font-mono break-all space-y-1">
            ...
            <div className="bg-err/10 border border-err/30 rounded-lg p-3.5 text-xs text-err font-mono break-all space-y-1">
              <div className="flex items-center gap-1.5 font-bold">
